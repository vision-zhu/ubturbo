...
static uint64_t GetRemoteNumaAvailablePages(int remoteNid)
{
    struct ProcessManager *manager = GetProcessManager();
    if (!manager || !IsRemoteNidValid(remoteNid)) {
        return 0;
    }
    int column = remoteNid - manager->nrLocalNuma;
...
static void CheckAndLimitMigrationByRemoteAvailable(ProcessAttr *process)
{
    if (GetRunMode() != WATERLINE_MODE || process->migrateMode != MIG_MEMSIZE_MODE) {
        return;
    }
    ...
    for (int l1Node = 0; l1Node < nrLocalNuma; l1Node++) {
        ...
        for (int j = 0; j < REMOTE_NUMA_NUM; j++) {
            int l2Node = nrLocalNuma + j;
            if (NotInAttrL2(process, l2Node)) {
                continue;
            }
            uint32_t migNum = process->strategyAttr.nrMigratePages[l1Node][l2Node];
            if (migNum == 0) {
                continue;
            }
            uint64_t available = GetRemoteNumaAvailablePages(l2Node);
            if (migNum > available) {
                SMAP_LOGGER_INFO("Pid %d migrate from %d to %d limited by remote available: %u -> %llu.",
                                 process->pid, l1Node, l2Node, migNum, available);
                process->strategyAttr.nrMigratePages[l1Node][l2Node] = (uint32_t)available;
            }
        }
    }
}
...
int RunStrategy(ProcessAttr *process, struct MigList mlist[MAX_NODES][MAX_NODES], size_t mlistSize)
{
    ...
    CheckAndLimitMigrationByRemoteAvailable(process);
    if (IsHugeMode()) {
