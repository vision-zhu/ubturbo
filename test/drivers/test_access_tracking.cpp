...
extern "C" int init_acpi_msg(struct acpi_msg *acpi_msg);
TEST_F(AccessTrackingTest, init_acpi_msg)
{
    int ret = init_acpi_msg(nullptr);
    EXPECT_EQ(-EINVAL, ret);

    struct acpi_msg acpi_msg;
    MOCKER(vzalloc).stubs().will(returnValue((void *)nullptr));
    ret = init_acpi_msg(&acpi_msg);
    EXPECT_EQ(-ENOMEM, ret);
}

TEST_F(AccessTrackingTest, init_acpi_msg_two)
{
    struct acpi_msg acpi_msg;
    acpi_mem.len = 1;
    ...
    int ret = init_acpi_msg(&acpi_msg);
    EXPECT_EQ(0, ret);
    list_del(&mem.segment);
    vfree(acpi_msg.acpi_seg);
}

extern "C" int init_iomem_msg(struct iomem_msg *iomem_msg);
TEST_F(AccessTrackingTest, init_iomem_msg)
{
    int ret = init_iomem_msg(nullptr);
    EXPECT_EQ(-EINVAL, ret);

    struct iomem_msg iomem_msg;
    MOCKER(get_remote_ram_len).stubs().will(returnValue(1));
    MOCKER(vzalloc).stubs().will(returnValue((void *)nullptr));
    ret = init_iomem_msg(&iomem_msg);
    EXPECT_EQ(-ENOMEM, ret);
}

TEST_F(AccessTrackingTest, init_iomem_msg_two)
{
    struct iomem_msg iomem_msg;
    ...
    int ret = init_iomem_msg(&iomem_msg);
    EXPECT_EQ(0, ret);
    list_del(&seg.node);
    vfree(iomem_msg.iomem_seg);
}
...
extern "C" int access_tracking_reinit_node(struct device *ldev);
TEST_F(AccessTrackingTest, access_tracking_reinit_node)
{
    struct access_tracking_dev adev;
    MOCKER(actc_buffer_reinit).stubs().will(returnValue(-1));
    int ret = access_tracking_reinit_node(&adev.ldev);
    EXPECT_EQ(-1, ret);

    GlobalMockObject::verify();
    adev.access_bit_actc_data = nullptr;
    MOCKER(actc_buffer_reinit).stubs().will(returnValue(0));
    ret = access_tracking_reinit_node(&adev.ldev);
    EXPECT_EQ(0, ret);
}
...
extern "C" int access_tracking_acpi_len_get(struct device *ldev, void __user *argp);
TEST_F(AccessTrackingTest, access_tracking_acpi_len_get)
{
    MOCKER(copy_to_user).stubs().will(returnValue(1UL));
    int ret = access_tracking_acpi_len_get(nullptr, nullptr);
    EXPECT_EQ(-EFAULT, ret);

    GlobalMockObject::verify();
    MOCKER(copy_to_user).stubs().will(returnValue(0UL));
    ret = access_tracking_acpi_len_get(nullptr, nullptr);
    EXPECT_EQ(0, ret);
}

extern "C" int access_tracking_iomem_len_get(struct device *ldev, void __user *argp);
TEST_F(AccessTrackingTest, access_tracking_iomem_len_get)
{
    MOCKER(get_remote_ram_len).stubs().will(returnValue(1));
    MOCKER(copy_to_user).stubs().will(returnValue(1UL));
    int ret = access_tracking_iomem_len_get(nullptr, nullptr);
    EXPECT_EQ(-EFAULT, ret);

    GlobalMockObject::verify();
    MOCKER(get_remote_ram_len).stubs().will(returnValue(1));
    MOCKER(copy_to_user).stubs().will(returnValue(0UL));
    ret = access_tracking_iomem_len_get(nullptr, nullptr);
    EXPECT_EQ(0, ret);
}

extern "C" int access_tracking_acpi_mem_get(struct device *ldev, void __user *argp);
TEST_F(AccessTrackingTest, access_tracking_acpi_mem_get)
{
    MOCKER(init_acpi_msg).stubs().will(returnValue(1));
    int ret = access_tracking_acpi_mem_get(nullptr, nullptr);
    EXPECT_EQ(1, ret);

    GlobalMockObject::verify();
    MOCKER(init_acpi_msg).stubs().will(returnValue(0));
    MOCKER(copy_to_user).stubs().will(returnValue(1UL));
    ret = access_tracking_acpi_mem_get(nullptr, nullptr);
    EXPECT_EQ(-EFAULT, ret);
...
extern "C" int access_tracking_iomem_get(struct device *ldev, void __user *argp);
static void *fake_buf_current = nullptr;
static int stub_init_iomem_msg(struct iomem_msg *msg)
{
    msg->cnt = 1;
    msg->iomem_seg = (struct iomem_seg *)fake_buf_current;
    return 0;
}

static void stub_vfree(void *p)
{
    free(p);
}

TEST_F(AccessTrackingTest, access_tracking_iomem_get)
{
    struct iomem_msg msg = {};
    msg.cnt = 1;

    MOCKER(init_iomem_msg)
        .stubs()
        .with(outBoundP((struct iomem_msg *)&msg, sizeof(msg)))
        .will(returnValue(1));

    int ret = access_tracking_iomem_get(nullptr, nullptr);
    EXPECT_EQ(1, ret);
    GlobalMockObject::verify();

    fake_buf_current = malloc(sizeof(struct iomem_seg));
    ASSERT_NE(fake_buf_current, nullptr);

    MOCKER(init_iomem_msg)
        .stubs()
        .will(invoke(stub_init_iomem_msg));
    ...
    MOCKER(vfree)
        .stubs()
        .with(eq(fake_buf_current))
        .will(invoke(stub_vfree));

    ret = access_tracking_iomem_get(nullptr, nullptr);
    EXPECT_EQ(-EFAULT, ret);
    GlobalMockObject::verify();
